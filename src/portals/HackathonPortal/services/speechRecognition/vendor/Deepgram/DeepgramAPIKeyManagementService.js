...
const KEY_SECURE_LOCAL_STORAGE_DEEPGRAM_API_KEY = "deepgram-api-key";

/**
 * Manages the subscription key acquisition and usage for Deepgram.
 */
export default class DeepgramSubscriptionKeyManagementService extends UIServiceCore {
  constructor(...args) {
    super(...args);
  ...
  async _setSecureLocalStorageSubscriptionKey(subscriptionKey) {
    await this._secureLocalStorageEngine.setItem(
      KEY_SECURE_LOCAL_STORAGE_DEEPGRAM_API_KEY,
      subscriptionKey
    );
  ...
  async deleteCachedSubscriptionKey() {
    if (
      !(await this.useServiceClass(UIModalWidgetService).confirm(
        "Are you sure you wish to delete the cached Deepgram API key?"
      ))
    ) {
      return;
    }
